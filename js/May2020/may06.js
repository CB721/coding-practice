...
const elevator = '^^^^^vv^v^vv^v^^^vvv^^^^^^v^^v^^^v^^vvv';
const elevator2 = '^^vvvv^^v^^v^^^^^';
const badElevator = '^^vv^^^^^^^^vReactRules!VV';

function whatFloor(str) {
    let floorLevel = 0;
    for (let i = 0; i < str.length; i++) {
        if (floorLevel < 0) {
            ...
            return "This elevator can't do that";
        }
    }
    return floorLevel;
}

console.log(whatFloor(elevator)); // 9
console.log(whatFloor(elevator2)); // your elevator crashed 😯
// Bonus
console.log(whatFloor(badElevator)); // This elevator can't do that